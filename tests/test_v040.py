...
def test_release_metadata_is_v040() -> None:
    assert __version__ == "0.68.9"
    assert 'version = "0.68.9"' in (ROOT / "pyproject.toml").read_text(encoding="utf-8")
    assert "## Version 0.68.9" in (ROOT / "README.md").read_text(encoding="utf-8")
...
def test_mt5_bridge_source_is_distributed() -> None:
    ...
    assert 'input string InpServerUrl = "http://127.0.0.1:8000";' in source

def test_mt5_get_uses_explicit_empty_dynamic_request_buffer() -> None:
    source = BRIDGE.read_text(encoding="utf-8")
    get_start = source.index("bool HttpGet(")
    post_start = source.index("bool HttpPost(")
...
def test_mt5_post_removes_string_terminator_before_webrequest() -> None:
    source = BRIDGE.read_text(encoding="utf-8")
    post_start = source.index("bool HttpPost(")
    post_end = source.index("int JsonInt(", post_start)
