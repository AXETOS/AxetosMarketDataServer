...
def create_app(
    ...
    def backfill(request: BackfillRequest) -> dict[str, object]:
        ...
        if kind == "mt5":
            provider = MetaTrader5TickProvider(worker.config.normalized_symbols(), worker.config.terminal_path, request.provider_key, symbol_aliases=worker.config.symbol_aliases, store=store)
...
form.onsubmit=async e=>{e.preventDefault();const d=new FormData(form),key=editing||d.get('provider_key'),kind=String(d.get('kind')),payload={provider_key:key,display_name:d.get('display_name'),kind,poll_interval_seconds:Number(d.get('poll_interval_seconds')),symbols:kind==='mt5'?editingSymbols:String(d.get('symbols')).split(',').map(x=>x.trim()).filter(Boolean),terminal_path:d.get('terminal_path')||null,priority:Number(d.get('priority')),fallback_after_seconds:Number(d.get('fallback_after_seconds')),batch_window_seconds:Number(d.get('batch_window_seconds')),batch_limit:Number(d.get('batch_limit')),maintenance_enabled:form.elements.maintenance_enabled.checked,maintenance_interval_minutes:Number(d.get('maintenance_interval_minutes')),maintenance_backfill_days:Number(d.get('maintenance_backfill_days')),feed_quiet_seconds:Number(d.get('feed_quiet_seconds')),feed_stalled_seconds:Number(d.get('feed_stalled_seconds')),feed_inactive_seconds:Number(d.get('feed_inactive_seconds')),enabled:form.elements.enabled.checked,auto_start:form.elements.auto_start.checked};const r=await fetch('/api/providers/'+encodeURIComponent(key),{method:'PUT',headers:{'content-type':'application/json'},body:JSON.stringify(payload)});if(!r.ok){error.textContent=(await r.json()).detail||r.statusText;return}editor.close();load()};load();loadEvents();setInterval(()=>{load();loadEvents()},3000);
</script></body></html>'''
