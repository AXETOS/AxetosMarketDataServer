...
def create_app(
    ...
    def provider_symbols(provider_key: str, refresh: bool = False, search: str | None = None, limit: int = Query(default=1000, ge=1, le=10000)) -> dict[str, object]:
        ...
        for item in discovered:
            policy=policies.get(str(item["provider_symbol"]))
            if policy:
                item.update({
            ...
            items.append(item)
        return {"provider_key": provider_key, "count": len(items), "items": items}
    ...
    def upsert_symbol_policy(request: SymbolPolicyRequest) -> dict[str, object]:
        ...
        if worker is not None and request.enabled and request.provider_symbol not in worker.config.normalized_symbols():
            worker.config.symbols = [*worker.config.normalized_symbols(), request.provider_symbol]
