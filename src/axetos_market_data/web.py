...
async function loadSymbols(refresh){if(!symbolProvider)return;symbolList.innerHTML='<div style="padding:16px">Loading…</div>';const q=new URLSearchParams({refresh:String(refresh),limit:'5000'});if(symbolSearch.value.trim())q.set('search',symbolSearch.value.trim());const r=await fetch('/api/providers/'+encodeURIComponent(symbolProvider)+'/symbols?'+q),x=await r.json();if(!r.ok){symbolList.innerHTML='<div class="error" style="padding:16px">'+esc(x.detail||'Could not load symbols')+'</div>';return}symbolSummary.textContent=`${x.count} symbols · automatic mappings remain disabled until reviewed`;symbolList.innerHTML=x.items.map((s,i)=>`<div class="symbol-row"><div><strong>${esc(s.provider_symbol)}</strong><small>${esc(s.description||'')}</small></div><input id="canon_${i}" value="${esc(s.canonical_instrument)}"><select id="state_${i}"><option value="confirmed" ${s.mapping_state==='Confirmed'?'selected':''}>Confirmed</option><option value="review" ${s.mapping_state==='NeedsReview'?'selected':''}>Needs review</option><option value="ignored" ${s.mapping_state==='Ignored'?'selected':''}>Ignored</option></select><input id="priority_${i}" type="number" min="0" placeholder="Priority" value="${s.priority_override??''}"><button onclick='saveSymbol(${JSON.stringify(s.provider_symbol)},${i})'>Save</button></div>`).join('')||'<div style="padding:16px">No symbols found.</div>'}
