...
def test_planner_checks_availability_before_download() -> None:
    m=FullHistoryBackfillManager(lambda *_:0,now_factory=lambda:datetime(2026,7,28,12,tzinfo=UTC)); m.start('P',[('EURUSD','EUR/USD')])
    assert complete_history_discovery(m, 'P').startswith('BACKFILL|EURUSD|1m|')
...
def test_unavailable_range_is_marked_bad_and_next_range_continues() -> None:
    ...
    req=complete_history_discovery(m, 'P').split('|'); m.batch_result('P',req[5],0,0,False,unavailable=True,error_code=4401)
