...
- Automatic startup replacement of the latest ten completed M1 candles remains unchanged.
...
Version 0.61.5 makes the recent 24-hour M1 refresh strictly sequential per instrument. The coordinator pauses after each symbol upload, verifies the authoritative row count and UTC minute alignment directly from persistent storage, reloads the same chart-facing read path, rebuilds H1/D1/W1/MN1 for that symbol, and only then dispatches the next instrument. Verification failures are persisted as bad ranges and do not freeze the provider workflow. Operational events now expose per-instrument COMPLETE/BAD outcomes and database/chart verification totals.
...
After all planned downloads are acknowledged, a bottom-up quality-aware repair run starts automatically. Stored ticks rebuild complete M1 candles where tick coverage is sufficient; complete M1 rebuilds H1; complete H1 rebuilds D1; complete D1 rebuilds W1 and MN1. Candle provenance records the source timeframe, quality rank, completeness, and repair run. A candidate overwrites an existing candle only when its deterministic quality rank is strictly better. Identical candidates are retained without a write; equal-quality disagreements are retained and counted as discrepancies; coarser candidates can never overwrite finer-derived candles. Operational events now report discovery completion, plan creation, download progress and totals, all-download completion, repair start, every repair stage, repair completion, bad-range totals, and final full-history completion. MT5 bridge v1.27 is unchanged.
